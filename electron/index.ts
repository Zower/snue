...
const createWindow = (): void => {
  ...
  ipcMain.on('fetch-url', (event, url) =>
    fetch(url, { redirect: "follow", follow: 100 }).then(r => r.text()).then(t => event.reply(`fetch-text-ready-${url}`, { type: 'Success', text: t, url })).catch(r => event.reply(`fetch-text-ready-${url}`, { type: "Error", error: r, url }))
  )
};
